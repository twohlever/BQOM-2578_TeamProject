...
# Enable full team to successfully execute locally 
...
df_CountyHealthAddl <- read_excel(paste(working_directory,"/data_raw/2025 County Health Rankings Data - v3.xlsx", sep = ""), skip = 1, sheet = "Additional Measure Data")
...
for (col in colsOfInterest) {
  p <- ggplot(df, aes_string(x = "r_COI_nat", y = col)) +
    geom_point(size = 2, aes(color = target_bin)) + 
    labs(x = "COI", y = col) +
    ggtitle(paste("Scatterplot of", col, "vs r_COI_nat")) +
    theme_minimal() + 
...
# No Hidden Layer
neuralzero = neuralnet(r_COI_nat~., 
                       data= scaled_train, hidden=0,
                       linear.output=TRUE, threshold=0.01, 
                       stepmax=20000, lifesign='full', lifesign.step=500)
...
total_ss <- sum((df[,3:5] - colMeans(df[,3:5]))^2)


iss <- function(k) {
  wss <- kmeans(df[,3:5], k, iter.max=100, nstart=100, algorithm="Lloyd")$tot.withinss
...
set.seed(1234)
k5<-kmeans(df[,c(3:5)],5,iter.max=100,nstart=50)
...
```
ggplot(df,aes(x=Age,y=SpendingScore,color=as.factor(ClusterNumber)))+geom_point(size=2)+
       labs(x = "Age", y = "Spending Score (1 to 100)") +
  ggtitle("Scatterplot") + theme_minimal()+
  theme(plot.title = element_text(hjust = 0.5))
#  scale_color_manual(values = c("0" = "white", "1" = "lightblue")) 


ggplot(df,aes(x=AnnualIncomek,y=SpendingScore,color=as.factor(ClusterNumber)))+geom_point(size=2)+
       labs(x = "Annual Income in thousands of dollars", y = "Spending Score (1 to 100)") +
  ggtitle("Scatterplot") + theme_minimal()+
  theme(plot.title = element_text(hjust = 0.5))
#  scale_color_manual(values = c("0" = "white", "1" = "lightblue")) 

ggplot(df,aes(x=Age,y=AnnualIncomek,color=as.factor(ClusterNumber)))+geom_point(size=2)+
       labs(x = "Age", y = "Annual Income in thousands of dollars") +
  ggtitle("Scatterplot") + theme_minimal()+
  theme(plot.title = element_text(hjust = 0.5))
